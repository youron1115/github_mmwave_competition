...
import System
from System import Array,UInt32, Int32, UInt16, Int16, String, UInt32, Int32, Byte
# from System import *
import numpy as np

Core_PATH = pathlib.Path(__file__).parent
sys.path.append(Core_PATH)
# For Single project test, you have to add this path to system environment, otherwise it will not relate to the package.
from KKT_Library.LibLog import lib_logger as log



DLL_NAME = "KSOC_Lib"
DLL_DIRS = ('KSOC_Libs', "KSOC_USB_Tools")
find_dll = False

for dir in DLL_DIRS:
    MAIN_PATH = pathlib.Path(__file__).parent.parent
    abs_path = os.path.normpath(os.path.join(MAIN_PATH, dir))
    if pathlib.Path(abs_path).joinpath(f"{DLL_NAME}.dll").is_file():
        log.info(abs_path)
...
class KSOCIntegration(KSOC_Integration):
    ...
    def switchSoftmaxInterrupt(self, enable, read_interrupt, clear_interrupt, size, ch_of_RBank, reg_addrs, frame_setting):
        ...
        :return: kres
        '''
        kres = self.SwitchSoftMaxInterruptAsserted(enable, read_interrupt, clear_interrupt, size, ch_of_RBank, reg_addrs, frame_setting)
        if kres != 0:
            raise KKTLibException(inspect.stack()[0][3], kres)
    ...
    def getSoftmaxInterruptAsserted(self, take_first_frame)->typing.Optional[dict]:
        ...
        '''
        kres, reg_vals = self.GetSoftMaxInterruptAssertedRegValues(None, take_first_frame)
        if kres != 0:
            return None
    ...
    def getDiagnosisInterruptAsserted(self, take_first_frame):
        ...
        :return: None or array of register's values.
        '''
        kres, raw, diagnosis, AGC, Motion, InterferenceBank = self.GetDiagnosisInterruptAssertedRegValues(None, None,
                                                                                                          None, None,
                                                                                                          None, take_first_frame)
        if kres != 0:
            return None
        if raw is not None:
    ...
    def getRXPhaseOffset(self):
        kres, rx_13_offset, rx_23_offset, temperature = self.GetRXPhaseOffset(Byte(0), Byte(0), Byte(0))
        if kres != 0:
            raise KKTLibException(inspect.stack()[0][3], kres)
    ...
    def readFromFlash(self, read_start_address: UInt32, read_size: int, read_align: int):
        kres, data = self.ReadDataFromFlash(read_start_address, read_size, read_align, None)
        if kres != 0:
            return None
        return data

    def readFromMemery(self, read_start_address: UInt32, read_size: int, read_align: int):
        kres, data = self.ReadDataFromMemery(read_start_address, read_size, read_align, None)
        if kres != 0:
            return None
        return data
