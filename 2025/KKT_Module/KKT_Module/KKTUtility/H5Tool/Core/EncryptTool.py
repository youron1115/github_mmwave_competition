...
class EncryptTool(object):
    ...
    @staticmethod
    def Get_public_Key():
        public_key = rsa.PublicKey(**_Keys._RSA_public_key)
        return public_key

    @staticmethod
    def Get_private_Key():
        private_key = rsa.PrivateKey(**_Keys._RSA_private_key)
        return private_key
    ...
    @staticmethod
    def compressBytesIOObject(BytesIOObject:BytesIO)-> BytesIO:
        btime = time.time()
        com_con = zlib.compress(BytesIOObject.getvalue())
        print("Compress time: {}".format(time.time() - btime))
        return BytesIO(com_con)
    @staticmethod
    def decompressBytesIOObject(BytesIOObject:BytesIO)-> BytesIO:
        btime = time.time()
        decom_con = zlib.decompress(BytesIOObject.read())
        BytesIOObject = BytesIO(decom_con)
        print("Decompress time: {}".format(time.time() - btime))
        return BytesIOObject
    ...
    def Encrypt_file_save(self, BytesIOObject, output_path):
        ...
        #
        # BytesIOObject.close()
        print('Save encrypted File : ', output_path)
